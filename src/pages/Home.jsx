    ...
    <div className="match-live-card p-4 relative overflow-hidden">
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] font-black text-live tracking-widest uppercase">Прямой эфир</span>
        </div>
        <span className="text-[10px] text-gray-400 uppercase tracking-wide">Группа {match.group} · {match.venue.split(',')[0]}</span>
      </div>

      <div className="flex items-center justify-between">
        <div className="flex flex-col items-center gap-1 flex-1">
          <span className="text-3xl">{home.flag}</span>
          <span className="text-xs font-bold text-white uppercase">{home.name}</span>
        </div>
        ...
        <div className="flex flex-col items-center gap-1 flex-1">
          ...
          <span className="text-xs font-bold text-white uppercase">{away.name}</span>
        </div>
      </div>
  ...
  return (
    <div className="match-finished-card p-3 flex items-center gap-3">
      ...
      <div className="flex items-center gap-2 flex-1 min-w-0">
        ...
        <span className="text-xs text-white truncate font-semibold uppercase">{home.name}</span>
      </div>
      ...
      <div className="flex items-center gap-2 flex-1 min-w-0 justify-end">
        <span className="text-xs text-white truncate text-right font-semibold uppercase">{away.name}</span>
  ...
  return (
    <div className="page-content px-4 pb-4">
      ...
      <div
        ...
        style={{
          background: 'linear-gradient(160deg, #0d1a3a 0%, #1a0d2e 40%, #080c15 100%)',
      ...
      >
        ...
        <div className="relative flex items-center justify-between">
          <div>
            ...
            <h1 className="text-2xl font-black text-white leading-tight tracking-wide uppercase">
              FIFA World Cup
            </h1>
            <p className="text-lg font-black" style={{ color: '#FFD700', letterSpacing: '0.08em' }}>
              2026™
            </p>
          </div>
          <div className="text-right">
            <div className="text-5xl trophy-glow">🏆</div>
            <div className="text-[10px] text-gray-400 mt-1 uppercase tracking-wider">11 июня 2026</div>
          </div>
        </div>
      ...
      <div
        ...
        style={{ background: '#0f1628', border: '1px solid rgba(255,215,0,0.3)', height: 36, borderRadius: 3 }}
      >
      ...
      {liveMatches.length > 0 && (
        <section className="mb-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => onTab('schedule')} className="text-[11px] font-bold uppercase tracking-wide" style={{ color: '#FFD700' }}>
              Все →
            </button>
          </div>
      ...
      {upcomingMatches.length > 0 && (
        <section className="mb-5">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-black text-white uppercase tracking-wider">Ближайшие матчи</h2>
            <button onClick={() => onTab('schedule')} className="text-[11px] font-bold uppercase tracking-wide" style={{ color: '#FFD700' }}>
              Расписание →
            </button>
          </div>
            ...
            {upcomingMatches.map((m) => {
              ...
              return (
                <div key={m.id} className="match-upcoming-card p-3 flex items-center gap-3">
                  <div className="text-[10px] text-gray-500 w-20 flex-shrink-0 leading-tight">
                    <div className="uppercase font-bold">{m.date}</div>
                    <div style={{ color: '#FFD700' }}>{m.time}</div>
                  </div>
                  <div className="flex items-center gap-2 flex-1 min-w-0 justify-center">
                    <span className="text-lg">{home.flag}</span>
                    <span className="text-xs font-bold text-white uppercase truncate">{home.name}</span>
                    <span className="text-xs text-gray-500 font-bold">vs</span>
                    <span className="text-xs font-bold text-white uppercase truncate">{away.name}</span>
                    <span className="text-lg">{away.flag}</span>
                  </div>
                  <div className="text-[9px] text-gray-600 uppercase tracking-wide flex-shrink-0">Гр.{m.group}</div>
                </div>
              )
            })}
          </div>
      ...
      <section className="mb-5">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-black text-white uppercase tracking-wider">Претенденты на Золотую бутсу</h2>
          <button onClick={() => onTab('scorers')} className="text-[11px] font-bold uppercase tracking-wide" style={{ color: '#FFD700' }}>
            Топ-15 →
          </button>
        </div>
        <div
          ...
          {TOP_SCORERS.slice(0, 3).map((s) => (
            <div key={s.rank} className="flex items-center gap-3 py-2">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-sm font-bold text-white truncate uppercase">{s.name}</div>
                <div className="text-[10px] text-gray-500">{s.club}</div>
              </div>
              <div className="flex items-center gap-1">
                <span className="text-xl font-black" style={{ color: '#FFD700' }}>{s.goals > 0 ? s.goals : '—'}</span>
                <span className="text-[10px] text-gray-500">голов</span>
              </div>
            </div>
          ))}
          <div className="mt-2 pt-2 border-t border-white/5 text-center text-[10px] text-gray-500 uppercase tracking-wider">
      ...
      <section className="mb-5">
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={() => onTab('insider')} className="text-[11px] font-bold uppercase tracking-wide" style={{ color: '#FFD700' }}>
            Все →
          </button>
        </div>
        <div className="space-y-3">
          {NEWS.filter((n) => n.hot).slice(0, 3).map((n) => (
            <div
              ...
              style={{ background: '#141929', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 3 }}
            >
              <div
