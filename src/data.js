...
// Фоновый баннер для шапок страниц — два слоя:
//   1) banner-flags.png — флаги хозяев. Жёстко прибиты к ВЕРХНЕМУ-ЛЕВОМУ углу,
//      высота = высоте баннера (size: auto 100% → ширина по пропорции). При cover-
//      обрезке основного арта на узких экранах флаги НЕ пропадают: это отдельный
//      слой слева. Меняется высота баннера → флаги повторяют её, оставаясь слева.
//   2) header-banner.jpg — основной арт (без флагов): чёрная зона слева, разлом,
//      белая карточка справа. cover + right top — белая карточка держится справа,
//      обрез уходит влево (под флаги) и вниз.
// В CSS первый слой рисуется ПОВЕРХ — поэтому флаги идут первыми.
// Без осветляющего оверлея: заголовки белые (на чёрном), данные справа чёрные (на белом).
export const HEADER_BANNER_STYLE = {
  backgroundImage: `url('${import.meta.env.BASE_URL}banner-flags.png'), url('${import.meta.env.BASE_URL}header-banner.jpg')`,
